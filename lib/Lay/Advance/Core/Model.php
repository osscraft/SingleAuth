...
abstract class Model extends SingleComponent implements Modelizable {
     ...
     * @param $unpk 
     */
    public final function save($use_last_id = true) {
        // TODO
        $pk = $this->primary();
        $colums = $this->columns();
        $pkl = array_search($pk, $columns);
     ...
     * @param $unpk 
     */
    public final function create($unpk = true, $use_last_id = true) {
        $pk = $this->primary();
        $colums = $this->columns();
        $pkl = array_search($pk, $columns);
        $data = $this->toArray();
        ...
        if(is_string($condition)) {
            ...
            if(is_array($condition)) {
                ...
                foreach ($condition as $table => $cond) {
                    ...
                    if(is_array($cond)) {
                        foreach ($cond as $f => $c) {
                            ...
                            if(in_array($f, $vcolumns)) {
                                ...
                                $f = $columns[$f];
                            } else if(is_numeric($f) && is_string($c)) {
                                $chip .= $c;
                            } else {
                                // ignore
                                continue;
                    ...
                    } else if(is_numeric($table) && is_string($cond)) {
                        $chip .= $cond;
                    }
                }
            }
